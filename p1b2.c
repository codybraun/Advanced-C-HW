...
searcher * search (double ** arrays, int n, int k){
  ...
  double previousMin, min = 100000.00;
  ...
  printf ("the size of arrays is %d", (int) sizeof(arrays[1][1]));
  ...
  for (x=0; x <n*k; x++){
      ...
      if (arrays[i][indexCounter[i]] == min)
	{
	  ...
	  printf ("updating pointers [%d][%d] to %d", y, i, returnSearcher-> pointers[y][i]);
	    indexCounter[i]++;
	 }
       else
	 {	   	   
	   returnSearcher -> pointers[y]= malloc(100);
	    returnSearcher -> pointers[y][i]=indexCounter[i];
	    printf ("not min, pointers [%d][%d] to %d", y, i, returnSearcher-> pointers[y][i]);
	 }
    } 
  } 
  printf ("at the end, [2][0] is %d", returnSearcher ->pointers[2][0]);
